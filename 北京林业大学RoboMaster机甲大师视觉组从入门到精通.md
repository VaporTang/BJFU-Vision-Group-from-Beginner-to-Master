...
- 雷达散热几乎不影响精度（但这并不意味着不需要考虑雷达的散热），导致定位大幅度漂移大概率是上位机性能不足
- 雷达的光学罩（Livox Mid-360 的那个紫色/深色光学罩）是雷达整个光学系统的一部分，如果沾染指纹或者积灰，激光束在发射和接收时会发生散射或折射。
  - 这可能会导致两种后果：一是有效探测距离变短；二是产生噪点
  - 清理应当先使用气吹或者压缩空气，吹除表面的硬质灰尘颗粒，再用无纺布蘸取异丙醇擦拭
- 点云数量和点云质量也影响定位性能
- IMU 与雷达的同步
  - 目前主要的激光 SLAM 都是 LIO，高度依赖 IMU 来去除雷达在运动中的畸变
    - 硬同步：通过硬件信号线让雷达和 IMU 的时间戳物理对齐，精度极高（微妙级），效果最好
